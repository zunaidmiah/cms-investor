(function (H) {
	...
	seriesTypes.trendline = H.extendClass(seriesTypes.line, {
		...
		**/
		...
		/* Function using the global SO function.
		 * 
		 * @return : an array of SO data.
		**/
		SOD: function (xData, yData, periods) {

			return SOD(xData, yData, periods);
		},
		 ...
		 * @return : an array of SO data.
		**/
		SOK: function (xData, yData, periods) {

			return SOK(xData, yData, periods);
		},
		 ...
		 * @return : an array of SO data.
		**/
		BBU: function (xData, yData, periods) {

			return BBU(xData, yData, periods);
		},
		 ...
		 * @return : an array of SO data.
		**/
		BBL: function (xData, yData, periods) {

			return BBL(xData, yData, periods);
		},
	...
	**/
	 ...
	 * @return an array containing the SMA.	
	**/
	function BBU (xData, yData, periods) {
		var periodArr = [],
			smLine = [],
			length = yData.length,
			sma=SMA(xData, yData, periods),
			pointStart = xData[0];
		...
		for (var i = 0; i < length; i++) {
			...
			if (periods == periodArr.length) {
				...
				smLine.push([ xData[i] , sma[i][1]+2*standardDeviation(periodArr)]);
				periodArr.splice(0,1);
	 ...
	 * @return an array containing the SMA.	
	**/
	function BBL (xData, yData, periods) {
		var periodArr = [],
			smLine = [],
			length = yData.length,
			sma=SMA(xData, yData, periods),
	 ...
	 * @return an array containing the SMA.	
	**/
	function SOD (xData, yData, periods) {
		var periodArr = [],
			highArr = [],
			lowArr = [],
			smLine = [],
			length = yData.length,
			sok=SOK(xData,yData,periods),
			pointStart = xData[0];
		/*// Loop through the entire array.
		for (var i = 0; i < length; i++) {
				highArr.push(yData[i][1]);
				lowArr.push(yData[i][2]);
			if (periods == highArr.length) {
				var so=(yData[i][3]-Math.min.apply(null,lowArr))/(Math.max.apply(null,highArr)-Math.min.apply(null,lowArr))*100;
				smLine.push([ xData[i] , so]);
				lowArr.splice(0,1);
				highArr.splice(0,1);
			}  else {
				smLine.push([ xData[i] , null]);
		...
		for(i=0;i<sok.length;++i)
			y.push(sok[i][1]);	
		return SMA(xData,y,3);
	}
	 ...
	 * @return an array containing the SMA.	
	**/
	function SOK (xData, yData, periods) {
		var periodArr = [],
			highArr = [],
			lowArr = [],
			smLine = [],
			...
			// 3: Remove first value.
				if(i==0){
					...
					if(yData[i-1]>=yData[i]){
						gain.push(yData[i]-yData[i-1]);
						loss.push(0);
					}
					else{
						loss.push(yData[i-1]-yData[i]);
						gain.push(0);
					}
				}
			if (periods == gain.length) {
				
				var g=arrayAvg(gain);
				var l=arrayAvg(loss);
				var rsi=(l==0)?100:(100-(100/(1+g/l)));
				smLine.push([ xData[i] , rsi]);
				if(gain.length>0) gain.splice(0,1);
				if(loss.length>0) loss.splice(0,1);
	...
	**/
	...
	function standardDeviation(values){
	  ...
	  var avgSquareDiff = arrayAvg(squareDiffs);
