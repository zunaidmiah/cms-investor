...
class AdminController extends BaseController {
		...
		if ($validator->fails()) {
			...
			// attempt to do the login
				...
				//echo $password; exit;
                            ...
                            Session::put("profilepic",json_decode($user,true)['profile_pic']); // Upwork code
    ...
    public function changepassword($user_id){
		...
		$password=DB::table('users')
			...
			if($password)
			{
				Session::flash('flash_message', '<i class="fa fa-check-circle">&nbsp;<b>Success!</b></i><br> The information has been saved successfully.');
    ...
    /**  Upwork code  This function is responsible for Uploading the avatar
        Directory : assets/images/profile/

   **/ 
    public function uploadavatar(){

          $userDetails = json_decode(Session::get("user"),true);

      if(Input::file("avatarfile")->isValid()){

        $path = Input::file('avatarfile')->move(public_path('assets/images/profile/'),Input::file('avatarfile')->getClientOriginalName());
        
        $updateProfilePic = DB::table("users")
                                ->where("id",$userDetails['id'])
                                ->update(["profile_pic" => "assets/images/profile/".Input::file('avatarfile')->getClientOriginalName()]);

         if($updateProfilePic){


            $profile_pic = "assets/images/profile/".Input::file('avatarfile')->getClientOriginalName();
            Session::put("profilepic",$profile_pic);

            echo json_encode(array("status" => "101","message" => "Successfully Uploaded Avatar"));
         
         }else{

              echo json_encode(array("status" => "102","error" => "Internal error"));
         }                       

      }else{


         echo json_encode(array("status" => "102","error" => "Upload a valid file"));
      }
