 ...
 public function saveCareer()
	{
		...
		if ($validator->fails()) {
			...
			$career->status      = $status;
                        $career->date      = Input::get('date');
                        $career->responsibilities  = Input::get('responsibilities_content');
                        $career->requirements         = Input::get('requirements_content');
                        $career->footertext        = Input::get('footer_content');
	...
	public function vacancy_set_paginate(){
        ...
        {
		$link = Career::truncate();
        	Session::flash('message', 'The vacancy has been deleted successfully!');
        	return Redirect::to('career_vac_edit');
        }
