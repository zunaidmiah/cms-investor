...
class IndexControllerV3 extends \BaseController {
	...
	public function index()
	{
        ...
        //     $stockdata          = DB::connection('charts')->select('SELECT * FROM ohlc ORDER BY `id` DESC LIMIT 0,1');
            $stockdata_from_klsescreener_charts = DB::connection('klsescreener_charts')->select('SELECT * FROM klsescreener_charts ORDER BY `id` DESC LIMIT 0,1');
            $stockdata_from_klsescreener_charts[0]->last_done = sprintf('%0.2f', $this->formatNumber($stockdata_from_klsescreener_charts[0]->last_done));
            $stockdata_from_klsescreener_charts[0]->market_cap = $this->formatNumber($stockdata_from_klsescreener_charts[0]->market_cap);

            $popup = PopUp::find(1);
            
            return View::make('front.indexv3')
                ...
                //     ->with('stockdata', $stockdata)
                    ->with('popup', $popup)
                    ->with('links', $links)
                    ->with('stockdata_from_klsescreener_charts', $stockdata_from_klsescreener_charts);
        }

        public function formatNumber($number){
            $sNum = $number.'';
            if(substr($sNum, -2) === "00") {
                $value = substr($sNum, 0, -2);
            }elseif(substr($sNum, -1) === "0"){
                $value = substr($sNum, 0, -1);
            }else{
                $value = $sNum;
            }
            return $value;
