...
class IndexControllerV2 extends \BaseController {
	...
	public function index()
	{
           ...
           // print_r($montages);
            ...
            $page_title="Majuperak Holdings Bhd | Investment Holding Company";
        ...
        //     $stockdata          = DB::connection('charts')->select('SELECT * FROM ohlc ORDER BY `id` DESC LIMIT 0,1');
            ...
            $stockdata_from_yahoo[0]->prev_close = $this->formatNumber($stockdata_from_yahoo[0]->prev_close);
            $stockdata_from_yahoo[0]->open = $this->formatNumber($stockdata_from_yahoo[0]->open);

            $popup = PopUp::find(1);
            
            return View::make('front.indexv2')
                ...
                //     ->with('stockdata', $stockdata)
                    ->with('popup', $popup)
                    ->with('links', $links)
                    ->with('stockdata_from_yahoo', $stockdata_from_yahoo);
        }
