		...
		foreach ($announcelists as $announce) {
			try {
				...
				switch ($cat) {
					...
					case "Changes in Director's Interest (Section 219 of CA 2016)":
		...
		if ($html['html']) {
			...
			try {
				...
				$view = View::make('template_general_announcments', array(
					'subtitle' => $title,
					'data' => $data,
					'attached' => 'https://bursa.fareastholdingsbhd.com/vendors/ckeditor/kcfinder/upload/files/' . $html['attachment_location_ondisk']
				));
