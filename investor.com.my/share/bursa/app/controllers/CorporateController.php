...
class CorporateController extends BaseController
{
  ...
  public function DirProfileAdd()
  {
    ...
    if (Input::hasFile('img')) {
      $file = str_random(10) . Input::file('img')->getClientOriginalName();
      Input::file('img')->move(public_path() . "/uploads/bods/", $file);
    }
    $inputs['img'] = $file;

    $diradd = Profile::create($inputs);
  ...
  public function DirProfileEdit()
  {
    ...
    // $sql = "ALTER TABLE profiles add `img` varchar(1000) null default null";
    // DB::statement($sql);
  ...
  public function FinancialInfoSegmentalAnalysis()
  {
    ...
    /* End of Paginate Count Section */
    $Profileslisting = Highlights::paginate($noperpage);
    if ($pgCount > 0) {
      $cntarray1 = $cntArr;
    } else {
      $cntarray1 = 0;
  ...
  public function AddFinancialInfoSegment()
  {
    // Create and save a new user, mass assigning all of the input fields (including the 'avatar' file field).
    $diradd = Highlights::create(Input::all());
    if ($diradd) {
      return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                  <button class="close" aria-hidden="true" data-dismiss="alert" type="button">�</button>
                  <i class="fa fa-check-circle"></i> <strong>Success!</strong>
  ...
  public function EditFinancialInfoSegment()
  {
    // Create and save a new user, mass assigning all of the input fields (including the 'avatar' file field).

    $proid = Input::get('id');
    $objHighlights = Highlights::find($proid);
    $objHighlights->year = Input::get('year');
    $objHighlights->earnpershare = Input::get('earnpershare');
    $objHighlights->netpershare = Input::get('netpershare');
    $objHighlights->currentratio = Input::get('currentratio');
    $objHighlights->pretaxprofit = Input::get('pretaxprofit');
    $objHighlights->pretaxshareholders = Input::get('pretaxshareholders');
    $objHighlights->active = Input::has('active') ? Input::get('active') : 0;

    if ($objHighlights->save()) {
      return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                  <button class="close" aria-hidden="true" data-dismiss="alert" type="button">�</button>
                  <i class="fa fa-check-circle"></i> <strong>Success!</strong>
                  <p>Edited Successfully.</p>
  ...
  public function DeleteAllFinancialInfoSegment()
  {
    $dirp = Highlights::truncate();
    if ($dirp) {
      return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                  <button class="close" aria-hidden="true" data-dismiss="alert" type="button">?</button>
                  <i class="fa fa-check-circle"></i> <strong>Success!</strong>
  ...
  public function SingleDeleteFinancialInfoSegment()
  {
    $id = Input::get('dirid');
    $dirpro = Highlights::findOrFail($id);
    $dirpro->delete();
    if ($dirpro) {
      return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
                  <button class="close" aria-hidden="true" data-dismiss="alert" type="button">�</button>
  ...
  public function DeleteMultipleFinancialInfo()
  {
    $id = Input::get('deleteid');
    $ids = explode(',', $id);
    $higlights = Highlights::whereIn('id', $ids)->delete();
    //  $banner->delete();
    if ($higlights) {
      return Redirect::back()->with('message', '<div class="alert alert-success alert-dismissable">
             <button class="close" aria-hidden="true" data-dismiss="alert" type="button">×</button>
  ...
  public function ManagementChairmansStat()
  {
    ...
    return View::make(
      'adminmanagementanalysischairmain',
      array(
        'page' => $page,
        'profilelist' => $Profileslisting,
        'cntarray1' => $cntarray1
  ...
  public function DirectorsFitAndProperPolicy()
  {
    ...
    $page = Page::where('type', '=', 'directorsfitandproperpolicy')->where('published', '=', 1)->get()->toArray();;
  ...
  public function frontDirectorsFitAndProperPolicy()
  {
    ...
    $page = Page::where('type', '=', 'directorsfitandproperpolicy')->where('published', '=', 1)->get()->toArray();
    $pageTitle = !empty($page)?$page[0]['page_title']:"";
    $masthead = url() . '/images/banner_subpage/banner13.jpg';
    $breadcrumbs = array(
      0 => array(
        "title" => "Home",
    ...
    $tagLine = "Oil Palm Plantation Investment Holdings";
    $mainMenuTitle = !empty($page)?$page[0]['page_title']:"";
    $metaTitle = $mainMenuTitle;
    return View::make(
  ...
  public function frontFinancialSegmentalAnalysis()
  {
    ...
    $page = Page::where('type', '=', 'financialinfosegmentalanalysis')->where('published', '=', 1)->get();
    $highlights = Highlights::where('active', '=', 1)->orderBy('created_at', 'DESC')->take(5)->get();
    $pageTitle = $page[0]->page_title;
    $masthead = url() . '/images/banner_subpage/banner13.jpg';
    $breadcrumbs = array(
      0 => array(
  ...
  public function DocumentSubmission()
  {
    ...
    return View::make('admindocumentsubmission', array(
      'page' => $page,
      'accountlisting' => $AdditionalListing,
      'cntarray1' => $cntarray1,
      'base_category' => $base_category
